...
import io.neo9.scaler.access.controllers.tcp.DynamicHttpRequestHandler;
import io.neo9.scaler.access.controllers.tcp.DynamicTcpRequestHandler;
...
import static io.neo9.scaler.access.config.Annotations.INTERCEPT_AS_TCP;
import static io.neo9.scaler.access.config.Commons.TRUE;
import static io.neo9.scaler.access.config.Labels.*;
...
public class EndpointSliceHijackingService {
    ...
    private final DynamicTcpRequestHandler dynamicTcpRequestHandler;
    private final DynamicHttpRequestHandler dynamicHttpRequestHandler;
    ...
    public EndpointSliceHijackingService(DynamicTcpRequestHandler dynamicTcpRequestHandler, DynamicHttpRequestHandler dynamicHttpRequestHandler, EndpointSliceRepository endpointSliceRepository, ServiceRepository serviceRepository, ScaleToZeroConfig scaleToZeroConfig, WorkloadService workloadService) {
        this.dynamicTcpRequestHandler = dynamicTcpRequestHandler;
        this.dynamicHttpRequestHandler = dynamicHttpRequestHandler;
    ...
    public void hijackIfNecessary(EndpointSlice appEndpointSlice) {
        ...
        if (isEmpty(appEndpointSlice.getEndpoints()) || appEndpointSliceIsPointingOnOldController(appEndpointSlice, controllerEndpointSlice)) {
            ...
            for (ServicePort servicePort : appService.getSpec().getPorts()) {
                ...
                if (TRUE.equals(getAnnotationValue(INTERCEPT_AS_TCP, appEndpointSlice))) {
                    dynamicTcpRequestHandler.startServer(servicePort.getPort());
                } else {
                    dynamicHttpRequestHandler.startServer(servicePort.getPort());
                }
            }
